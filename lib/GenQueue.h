...
typedef int (*ActionFunc)(void *_element, void *_context);

/**
 * @brief Create a new queue with a size of _size
 * @param[in] _size - The size of the queue
 * @return Queue pointer - on success, NULL on failure or if given size is 0
 */
 ...
 * @return size_t - return 1 if empty, 0 if not empty
 */
 ...
 * @return size_t - the number of times action was implemented (between 1 and num of elements), num of elements + 1 if element
 * 				    was not found, 0 if the given queue or action function are null
 * @details if action returns 0 stop running and return the number of times action run
 */
size_t QueueForEach(Queue *_queue, ActionFunc _action, void *_context);
