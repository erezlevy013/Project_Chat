 ...
 * @retval HEAP_NOT_INITIALIZED error if either pointer was not initialized
 * @retval HEAP_VECTOR_REALLOCATION_FAILED error, heap could not reallocate underlying vector 
 * @retval HEAP_VECTOR_OVERFLOW_ERROR if underlying vector's block size is 0
 */
 ...
 * @return pointer to element, can be null if heap is empty (null or no elements). 
 */
 ...
 * @return pointer to element, can be null if heap is empty (null or no elements) or if VectorRemove failed. 
 */
